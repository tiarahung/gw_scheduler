...
class Swope(Telescope):
    ...
    def compute_gw_exposure(self, gw):
        exposures = {}
        exposures.update({Constants.r_band: gw.exp})
        gw.exposures = exposures

    def compute_exposures(self):
        for tgt in self.targets:

            total_possible_time = len(np.where(tgt.raw_airmass_array <= Constants.airmass_threshold)[0])
            ...
            if total_possible_time > 0:
                ...
                fudge_factor = 100 if len(tgt.exposures) > 3 else 100 # Build in a fudge factor based on # of exps GW
            ...
            for t in targets:
                ...
                if (last_filter == Constants.r_band) or \
                   (last_filter == Constants.i_band) or \
                    ...
                    output_rows.append(self.swope_filter_row(Constants.r_band, t.exposures[Constants.r_band]))  #comment out for GW
                    # output_rows.append(self.swope_filter_row(Constants.i_band, t.exposures[Constants.i_band]))
                    # output_rows.append(self.swope_filter_row(Constants.g_band, t.exposures[Constants.g_band]))  #comment out for GW
...
class Nickel(Telescope):
    ...
    def compute_gw_exposure(self, gw):
        exposures = {}
        exposures.update({Constants.r_prime: 90})
    ...
    def compute_exposures(self):
        for tgt in self.targets:
            ...
            if total_possible_time > 0:
                ...
                fudge_factor = 100 if len(tgt.exposures) > 2 else 100 # Build in a fudge factor based on # of exps
...
class Keck(Telescope):
    ...
    def compute_gw_exposure(self, gw):
        exposures = {}
        exposures.update({Constants.g_band: 300})
    ...
    def keck_filter_row(self, exp_name, exp_time):
        filter_row = []
        filter_row.append(None)
        filter_row.append(None)
        filter_row.append(None)
            ...
            for t in targets:
                ...
                if (last_filter == Constants.r_prime) or \
                   (last_filter == Constants.i_prime) or \
                    ...
                    # Start in r'i'VB order
                    output_rows.append(self.keck_filter_row(Constants.g_band, t.exposures[Constants.g_band]))
                    # output_rows.append(self.nickel_filter_row(Constants.i_prime, t.exposures[Constants.i_prime]))
                    last_filter = Constants.i_prime

                    if len(t.exposures) > 2:
                        output_rows.append(self.keck_filter_row(Constants.V_band, t.exposures[Constants.V_band]))
                        output_rows.append(self.keck_filter_row(Constants.B_band, t.exposures[Constants.B_band]))
                        last_filter = Constants.B_band
                ...
                # Flip order: BVi'r'
                    ...
                    output_rows.append(self.keck_filter_row(Constants.B_band, t.exposures[Constants.B_band]))
                    output_rows.append(self.keck_filter_row(Constants.V_band, t.exposures[Constants.V_band]))
                    output_rows.append(self.keck_filter_row(Constants.i_prime, t.exposures[Constants.i_prime]))
                    output_rows.append(self.keck_filter_row(Constants.r_prime, t.exposures[Constants.r_prime]))
                    last_filter = Constants.r_prime
...
#     def compute_gw_exposure(self, gw):
#         exposures = {}
#         exposures.update({Constants.i_band: 90})
...
#     def ANDICAM_filter_row(self, exp_name, exp_time):
#         filter_row = []
#         filter_row.append(None)
#         filter_row.append(None)
#         filter_row.append(None)
